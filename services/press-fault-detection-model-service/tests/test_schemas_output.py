...
class TestPredictionResponse:
    ...
    def test_valid_prediction_response_creation(self):
        ...
        assert response.fault_probability == 0.01
    ...
    def test_valid_prediction_response_with_attribute_errors(self):
        ...
        assert response.fault_probability == 0.9
    ...
    def test_prediction_response_with_empty_attribute_errors(self):
        ...
        assert response.is_fault is False
        assert response.fault_probability == 0.0
        assert response.attribute_errors == {}
    ...
    def test_prediction_response_json_serialization(self):
        ...
        assert '"fault_probability":0.02' in json_str
    ...
    def test_missing_required_field_fault_probability(self):
        """Test validation error when fault_probability field is missing."""
        data = {
            "prediction": "normal",
            "reconstruction_error": 0.05,
            "is_fault": False
        ...
        assert "fault_probability" in str(exc_info.value)
        assert "Field required" in str(exc_info.value)
    ...
    def test_invalid_fault_probability_type(self):
        """Test validation error when fault_probability is not a float."""
        data = {
            "prediction": "normal",
            "reconstruction_error": 0.05,
            "is_fault": False,
            "fault_probability": "not_a_float"
        }
    ...
    def test_fault_probability_out_of_range(self):
        """Test validation error when fault_probability is not between 0 and 1."""
        invalid_data = [
            {"fault_probability": -0.1},
    ...
    def test_model_immutability(self):
        """Test that model instances are mutable by default."""
        data = {
            "prediction": "normal",
            "reconstruction_error": 0.05,
            "is_fault": False,
        ...
        # Pydantic models are mutable by default, so we can test assignment
    ...
    @pytest.mark.parametrize("prediction,reconstruction_error,is_fault,fault_probability", [
        ("normal", 0.01, False, 0.0),
        ("fault", 0.99, True, 0.9),
        ("anomaly", 0.5, True, 0.5),
        ("healthy", 0.0, False, 0.0),
        ("critical", 1.0, True, 1.0),
    ])
    def test_various_valid_combinations(self, prediction, reconstruction_error, is_fault, fault_probability):
        ...
        assert response.fault_probability == fault_probability
        assert response.attribute_errors is None
    ...
    def test_model_schema(self):
        ...
        assert "fault_probability" in schema["properties"]

        # Check required fields
        assert "required" in schema
        required_fields = {"prediction", "reconstruction_error", "is_fault", "fault_probability"}
        assert set(schema["required"]) == required_fields

        # Check field types
        assert schema["properties"]["prediction"]["type"] == "string"
        assert "anyOf" in schema["properties"]["reconstruction_error"]
        assert schema["properties"]["is_fault"]["type"] == "boolean"
        assert schema["properties"]["fault_probability"]["type"] == "number"
