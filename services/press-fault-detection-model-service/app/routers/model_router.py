    ...
    try:
        ...
        model_info = {
            ...
            "status": {
                ...
                "all_components_ready": all([model_loaded, scaler_loaded, threshold_loaded])
            },
            "input_features": [
                "AI0_Vibration",
                "AI1_Vibration", 
        ...
        if threshold_loaded:
            try:
                model_info["threshold_value"] = float(threshold) if threshold is not None else None
            except Exception:
        ...
        raise HTTPException(status_code=500, detail=f"모델 정보 조회 중 오류 발생: {str(e)}") from e