...
from app.utils.test_logger import AnomalyLogger, anomaly_logger
...
class TestAnomalyLogger(unittest.TestCase):
    ...
    @patch('app.utils.test_logger.settings')
    def test_init_creates_log_directory(self, mock_settings):
        ...
        self.assertEqual(logger.log_file_path, os.path.join(self.temp_dir, "test.log"))

    @patch('app.utils.test_logger.settings')
    @patch('app.utils.test_logger.datetime')
    ...
    def test_log_anomaly_writes_to_file_and_console(self, mock_print, mock_datetime, mock_settings):
        ...
        # Execute
        logger.log_anomaly(self.service_name, self.sample_prediction_result, self.sample_original_data)
        ...
        self.assertEqual(log_entry["prediction"], self.sample_prediction_result)
        ...
        expected_calls = [
            ...
            unittest.mock.call(f"   └─ MAE: {self.sample_prediction_result['mae']}"),
            unittest.mock.call(f"   └─ Threshold: {self.sample_prediction_result['threshold']}"),
            unittest.mock.call(f"   └─ Status: {self.sample_prediction_result['status']}"),
    ...
    def test_log_anomaly_handles_missing_prediction_fields(self, mock_print, mock_settings):
        ...
        logger.log_anomaly(self.service_name, incomplete_prediction, self.sample_original_data)
    ...
    def test_log_normal_processing_console_only(self, mock_print, mock_settings):
        ...
        logger.log_normal_processing(self.service_name, self.sample_prediction_result)
    ...
    def test_log_error_writes_to_error_file(self, mock_print, mock_datetime, mock_settings):
        ...
        logger.log_error(self.service_name, error_message, self.sample_original_data)
    ...
    def test_unicode_handling_in_logs(self, mock_settings):
        ...
        logger.log_anomaly(unicode_service, self.sample_prediction_result, unicode_data)
    ...
    def test_empty_prediction_result(self, mock_settings):
        ...
        # Should not crash
        logger.log_anomaly(self.service_name, empty_prediction, self.sample_original_data)
...
class TestAnomalyLoggerIntegration(unittest.TestCase):
    ...
    def test_full_logging_workflow(self, mock_settings):
        ...
        # Test error logging
        logger.log_error("integration_test", "Integration test error", original_data)
