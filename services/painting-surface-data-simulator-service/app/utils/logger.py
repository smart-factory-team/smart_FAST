...
class AnomalyLogger:
    ...
    def log_anomaly(self, service_name: str, prediction_result: Dict[str, Any], original_data: Dict[str, Any]):
        ...
        # 콘솔 출력
        print(f"🚨 DEFECT DETECTED: {service_name}")
        print(f"   └─ Defect Count: {prediction_result.get('defect_count', 'N/A')}")
        print(f"   └─ Total Images: {prediction_result.get('total_count', 'N/A')}")
    ...
    def log_normal_processing(self, service_name: str, prediction_result: Dict[str, Any]):
        """정상 처리 결과를 콘솔에만 출력"""
        print(
            f"✅ NORMAL: {service_name} - 결함 없음 (총 {prediction_result.get('total_count', 'N/A')}개 이미지)")
