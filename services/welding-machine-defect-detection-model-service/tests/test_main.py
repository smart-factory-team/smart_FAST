...
class TestFastAPIMainApplication:
    ...
    def test_router_prefix_configuration(self):
        ...
        assert response.status_code in [
            405, 422], f"Expected 405 or 422, got {response.status_code}"
    ...
    def test_content_type_handling(self):
        ...
        response = self.client.get(
            "/openapi.json", headers={"Accept": "application/json"})
        assert response.status_code == 200
    ...
    def test_app_openapi_version_compatibility(self):
        ...
        assert openapi_version.startswith(
            "3."), f"Expected OpenAPI 3.x, got {openapi_version}"
...
class TestApplicationConfigurationEdgeCases:
    ...
    def test_signal_type_validation(self, signal_type, expected_validation):
        ...
        if signal_type == "cur":
            values = [1.0] * 1024
        elif signal_type == "vib":
            values = [1.0] * 512
        else:
            values = [1.0] * 3  # 임의의 짧은 길이
